...
- Pin container images to explicit release or patch tags; never use `latest`.
...
- Default version: `RELEASE.2025-07-23T15-54-02Z`
...
- Default version: `RELEASE.2025-07-21T05-28-08Z`
...
MINIO_CLIENT_VERSION=RELEASE.2025-07-21T05-28-08Z
...
- The project must not silently fall back to unpinned image versions.
...
2. PostgreSQL, Redis, MinIO, and Mailpit reach `healthy` status.
3. `minio-init` exits successfully with code 0.
4. PostgreSQL accepts a connection to `booking_os` with the configured user.
5. Redis responds with `PONG`.
6. The MinIO default bucket exists and is private.
7. Mailpit UI responds on the configured UI port.
8. `docker compose down` preserves named-volume data.
9. `docker compose down --volumes` removes local data.
10. No `.env.docker` file or local data directory is committed.
