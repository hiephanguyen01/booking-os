...
export class CompletePasswordResetUseCase {
  ...
  async execute(command: CompletePasswordResetCommand): Promise<CompletePasswordResetResult> {
    ...
    const result = await this.repository.replacePasswordAndConsumeReset({
