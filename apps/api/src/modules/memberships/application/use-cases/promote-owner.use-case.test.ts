...
import { PromoteOwnerUseCase } from "./promote-owner.use-case.js";
...
const authorization: AuthorizationContext = {
  ...
  permissionKeys: [PERMISSION_KEYS.tenantMembershipOwnerPromote],
...
function createTransactions(events: string[]): TenantTransactionPort {
  return {
    async run(context, work) {
      ...
      assert.equal(context.requestId, "request-promote-owner");
      return work({
        memberships: {
        ...
        audit: {
          append: async (input: unknown) => {
            assert.deepEqual(input, {
              ...
              requestId: "request-promote-owner",
              metadata: {
...
test("promotes an active admin, bumps authorization once, and revokes tenant sessions", async () => {
  const events: string[] = [];
  const useCase = new PromoteOwnerUseCase(createTransactions(events), () => NOW);
...
test("rejects owner promotion without owner-promote permission", async () => {
  const useCase = new PromoteOwnerUseCase(createTransactions([]), () => NOW);
  ...
  await assert.rejects(
    () =>
      useCase.execute({
        ...
        membershipId: MEMBERSHIP_ID,
        requestId: "request-promote-denied",
      }),
