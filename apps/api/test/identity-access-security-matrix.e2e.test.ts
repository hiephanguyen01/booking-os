...
identityAccessTest(
  "S1B-AC01 bootstrap provisions the first platform administrator idempotently",
  () => {
...
identityAccessTest(
  "S1B-AC02 platform authentication is bound to trusted assignment and exact host",
  () => {
...
identityAccessTest(
  "S1B-AC03 platform provisioning creates the tenant and first-owner invitation atomically",
  () => {
...
identityAccessTest(
  "S1B-AC05 an existing global user joins another tenant without a second identity",
  () => {
...
identityAccessTest(
  "S1B-AC06 sessions remain host-bound and browser cookies use the host-only contract",
  () => {
...
identityAccessTest(
  "S1B-AC07 tenant or platform session material cannot replay on the wrong host or scope",
  () => {
...
identityAccessTest(
  "S1B-AC09 password reset is enumeration-safe, single-use, short-lived, and revokes sessions",
  () => {
...
identityAccessTest(
  "S1B-AC11 current authorization is authoritative, current-scope-only, and no-store",
  () => {
...
identityAccessTest(
  "S1B-AC13 state-changing browser requests require approved origin and CSRF proof",
  () => {
...
identityAccessTest(
  "S1B-AC14 raw secrets stay out of persistence, audit, responses, and outbox payloads",
  () => {
